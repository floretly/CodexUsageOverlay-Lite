...
- 默认字号下基础宽度为 720 px，并会按实际内容和字号自动扩展；组件窗口保持居中，内容组在中心基础上做轻微右移的光学校正。
- 标题栏外层背景完全透明；短周期额度、周额度、重置券、Token 和重置雷达采用同基线纯文本分区显示，关键数值使用粗体，所有标题栏文字颜色继续由“外观”和“自定义颜色”设置统一控制。
- 设置入口以浅灰齿轮常驻显示在状态文字右侧，并使用细分隔线与雷达状态区分。
- 显示 5 小时额度及重置时间、周剩余及周重置时间和重置券。
...
- 程序启动后会定期检查本仓库的最新稳定 Release。
- 发现新版本时会显示更新提示；点击后可确认下载并自动安装。
- 下载使用 HTTPS，并在安装前对安装包执行 SHA-256 校验；校验失败不会继续安装。
...
- 字号：7.5–11 pt，以 0.5 pt 为步进调整；雷达文字会自动保持略小一级，悬浮条宽度和左右留白会随字号同比例变化。
...
直接下载：
https://github.com/floretly/CodexUsageOverlay-Lite/releases/download/v1.0.17/CodexUsageOverlay-Lite-Setup-1.0.17.exe

```text
CodexUsageOverlay-Lite-Setup-1.0.17.exe
```
...
```powershell
.\CodexUsageOverlay-Lite-Setup-1.0.17.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART /CLOSEAPPLICATIONS
```
